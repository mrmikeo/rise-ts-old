...
export interface MultiSignaturesAPI {
     ...
     */
    getPending(publicKey: string, callback?: cback<{
        transactions: {
            min: number;
            max: number;
            lifetime: number;
            signed: true;
            transaction: MultiSigTransaction<any>;
        }[];
    }>): Promise<{
        transactions: {
            min: number;
    ...
    createMultiSigAccount(sig: {
        secret: string;
        lifetime: number;
        min: number;
        publicKeys: string[];
    }, callback?: cback<any>): any;
    sign(obj: {
        secret: string;
        publicKey: string;
        transactionId: string;
    }, callback?: cback<any>): any;
    getAccounts(publicKey: string, callback?: cback<any>): any;
}
